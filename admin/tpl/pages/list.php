				...
				<table>
					<thead>
						<tr>
							...
							<th style="text-align:center;" width="50">置顶</th>
						...
						if ($data['list'] && is_array($data['list'])) {
							foreach ($data['list'] as $val) {
								...
								<tr id="itemlist_<?= $val['id'] ?>">
									...
									<td style="text-align:center;"><div class="input"><a href="javascript:;" onclick="recommend(this, '<?= $val['id'] ?>', '<?= $m ?>', '<?= $SKIN ?>')"><img src="<?= $SKIN ?>img/icons/<?= $val['sort'] ? "button-check" : "button-white-remove" ?>.png" width="20"  title="推荐" /></a></div></td>
									<td class="actions">
										<a href="?m=<?= $m ?>&c=edit&itemid=<?= $val['id'] ?>"><img src="<?= $SKIN ?>img/icons/actions/edit.png" title="修改"  /></a>
										<a  href="javascript:;" onclick="itemremove('?m=<?= $m ?>&c=ajax&c=remove&itemid=<?= $val['id'] ?>', true)" ><img src="<?= $SKIN ?>img/icons/actions/delete.png" title="删除" /></a>
									</td>
								</tr>
								<?php
							}
