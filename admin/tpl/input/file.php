...
<script language="javascript">
	KindEditor.ready(function(K) {
		...
		K('#detail-<?=$FIELD['name']?>-btn').click(function() {
			editor_<?=$FIELD['name']?>.loadPlugin('insertfile', function() {
				editor_<?=$FIELD['name']?>.plugin.fileDialog({
					fileUrl : K('#detail-<?=$FIELD['name']?>').val(),
...
<div class="input">
	...
	<div class="update-box">
		...
		<input type="button" style="width:70px;" id="detail-<?=$FIELD['name']?>-btn" class="update-btn" value="选择文件" />
	</div>
