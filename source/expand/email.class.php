...
class email {
	...
	public function __construct($config) {
		$this->System = $config;
		$this->_SC = $config->_SC;
		$emailUserName = $this->_SC['emailUserName'];
		$emailPassword = $this->_SC['emailPassword'];

		$this->_sendServer = $this->_SC['smtpHost'];
		$this->_port = $this->_SC['smtpPort'];
		$this->_isSecurity = $this->_SC['smtpSecurity'];
		$this->_userName = empty($emailUserName) ? "" : base64_encode($emailUserName);
		$this->_password = empty($emailPassword) ? "" : base64_encode($emailPassword);
	}
