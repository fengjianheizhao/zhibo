...
class mysql {
	...
	private $pattern;  //数据库连接标识;
	private $result;   //执行query命令的结果资源标识
	public $sql;   //sql执行语句
	private $charset;  //数据库编码，GBK,UTF8,gb2312
	private $bulletin = DEBUG; //是否开启错误记录
	private $show_error = DEBUG;  //测试阶段，显示所有错误,具有安全隐患,默认关闭
	...
	public function __construct($system) {
		$this->System = $system;
		$this->DB_host = $system->_SC['dbhost'];
		$this->DB_user = $system->_SC['dbuser'];
		$this->DB_pwd = $system->_SC['dbpw'];
		$this->DB_database = $system->_SC['dbname'];
		$this->pattern = $system->_SC['dbpattern'];
		$this->charset = $system->_SC['dbcharset'];
		$this->conn = "ODBC"; //"ODBC", "pconn"
		$this->connect();
	}
	...
	/* 取得记录集,获取数组-索引和关联,使用$row['content']   $row->content */
	...
	public function get_one($sql) {
		$this->query($sql);
		$result = $this->fetch_assoc($this->result);
		if ($result) {
			return $result;
		} else {
			return array();
	...
	public function get_result($sql) {
		$this->query($sql);
		while ($res = $this->fetch_assoc($this->result)) {
			$result[] = $res;
		}
		if ($result) {
			return $result;
	...
	//简化修改update
	public function update($table, $content, $condition) {
		$mod_content = $p = "";
	...
	public function show_error($message = "", $sql = "") {
		if (!$sql) {
			...
			if ($this->bulletin) {
				...
				$filename = $server_date . ".php";
				$file_path = "mysql_error/" . $filename;
				$error_content = $message;
				//$error_content="错误的数据库，不可以链接";
				$file = "mysql_error"; //设置文件保存目录
				//建立文件夹
				if (!file_exists($file)) {
					if (!mkdir($file, 0777)) {
